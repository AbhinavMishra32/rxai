...
export default function TodoCard(props) {
    ...
    return (
        <li key={props.todoIndex}>
            {children}
            <div className="bg-stone-800 m-3 border border-stone-600 p-3 rounded-2xl flex items-center bg-gradient-to-br from-stone-900 to-stone-800 transition-all ease-in-out">
                <input type="checkbox" className='relative peer shrink-0 appearance-none w-6 h-6 border-2 border-stone-500 rounded-md bg-stone-600 focus:ring-offset-stone-600 focus:ring-2  hover:bg-stone-500  hover:border-stone-700 focus:ring-stone-300 transition-all ease-in-out checked:bg-stone-300 '
                    onClick={() => { handleDeleteTodos(index) }} />
                <p className="ml-4 font-unbounded text-stone-300 truncate">{props.todos}</p>
